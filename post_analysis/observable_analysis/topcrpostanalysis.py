	...
	def _normalize_Q(self):
		...
		for atype in self.analysis_types:
			for beta in self.beta_values:
				...
				self.topc2[atype][beta]["y_error"] = np.sqrt(
					(self.topc2[atype][beta]["y_error"]/self.V[beta])**2 +\
					(self.V_err[beta]*self.topc2[atype][beta]["y"]/self.V[beta]**2)**2)
				self.topc2[atype][beta]["y"] /= self.V[beta]

				# self.topc4[atype][beta]["y_error"] /= self.V[beta]**2
				self.topc4[atype][beta]["y_error"] = np.sqrt(
					(self.topc4[atype][beta]["y_error"]/self.V[beta]**2)**2 +\
					(2*self.V_err[beta]*self.topc4[atype][beta]["y"]/self.V[beta]**3)**2)
