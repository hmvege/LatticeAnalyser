...
def observable_plotter(feps_dictionary, observable, x_limit=0, 
    x_label=r"$t_f$", y_label="", savefig_folder="", exclude_eps=[]):
    ...
    fig_name = os.path.join(savefig_folder, "{0:s}.png".format(observable))
...
def get_differences(feps_dictionary, observable, diff="relative", x_limit=0, 
            ...
            for iv, _xv in enumerate(xvals):
                ...
                if np.abs(_xv - _xmin) < eps:
                    ...
                    if diff == "absolute":
                        ...
                        _y.append(np.abs((y_min[imin] - yvals[iv])/y_min[imin]))
                    else:
                        raise KeyError(
    ...
    fig_name = os.path.join(savefig_folder,
        "{0:s}_diff_{1:s}.png".format(observable, diff))
    if diff == "absolute":
...
def main():
    ...
    for obs in observables:
        observable_plotter(feps_dict, obs, x_limit=1.0, 
            x_label=observables[obs]["x"], y_label=observables[obs]["y"],
